...
function Curve({ history }: { history: JobState["history"] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="curve">
      <path d={line((h) => h.train_loss)} fill="none" stroke="#38bdf8" strokeWidth="2" />
      <path d={line((h) => h.val_loss)} fill="none" stroke="#f472b6" strokeWidth="2" strokeDasharray="5 3" />
      {history.map((h, i) => (
        <circle key={i} cx={X(i)} cy={Y(h.train_loss)} r="2.5" fill="#38bdf8" />
      ))}
      <text x={P} y={14} fill="#94a3b8" fontSize="11">— train_loss</text>
      <text x={P + 90} y={14} fill="#94a3b8" fontSize="11">- - val_loss</text>
    </svg>
  );
}
...
export default function LogsPanel({ job, logs, open, onToggle, summary }: {
  ...
  return (
    <section className={`logs${open ? "" : " closed"}`}>
      <div className="logs-head" onClick={onToggle} title={open ? "Collapse" : "Expand"}>
        ...
        {job?.params != null && <span className="muted small">{job.params} params</span>}
        {job?.dataset_info && <span className="muted small">{String((job.dataset_info as { source?: string }).source)}</span>}
      </div>
      <div className="logs-body">
        <div className="logs-left">
          ...
          {job && job.history.length > 0 && (
            <table>
              <thead><tr><th>epoch</th><th>train</th><th>val</th><th>acc</th></tr></thead>
              <tbody>
                {job.history.slice(-8).map((h) => (
                  <tr key={h.epoch}><td>{h.epoch}</td><td>{h.train_loss}</td><td>{h.val_loss}</td><td>{h.val_acc}</td></tr>
                ))}
              </tbody>
            </table>
          )}
